...
public void mirrorHorizontalBottoTop() {
    ...
    for(int i = 0; i < pixels.length/2; i++) {
      for(int j = 0; j < pixels[i].length/2; j++) {
        pixels[i][j].setRed(pixels[pixels.length-i][pixels[i].length-j].getRed());
        pixels[i][j].setGreen(pixels[pixels.length-i][pixels[pixels.length-i].length-j].getGreen());
        pixels[i][j].setBlue(pixels[pixelts.length-i][pixels[pixels.length-i].length-j].getBlue());
      }
    }
}
